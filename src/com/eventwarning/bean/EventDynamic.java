...
public class EventDynamic {
	...
	public double getHotDegree(EventDynamic ed){
		double ans = 0;
		ans += (this.weiboNum - ed.weiboNum)*0.2;
		ans += (this.commentNum - ed.commentNum)*0.2;
		ans += (this.repostNum - ed.repostNum)*0.2;
		ans += (this.vUserNum - ed.vUserNum)*0.2;
		ans += (this.uniqueUserNum - ed.uniqueUserNum)*0.2;
		return ans;
	}
	public EventDynamic(Map row){
		this.dynamicID = Integer.parseInt(row.get("dynamicID").toString());
		this.event = DBOperation.GetEventInfo(Integer.parseInt(row.get("eventID").toString()));// <-- 修改
