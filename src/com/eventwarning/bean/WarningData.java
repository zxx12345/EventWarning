...
public class WarningData {
	...
	public double getHotD(EventDynamic ed){
		double ans = 0;
		if(maxWeiboNum>minWeiboNum)
		ans += (ed.weiboNum-minWeiboNum)/(maxWeiboNum-minWeiboNum)*0.2;
		if(maxUniqueUserNum>minUniqueUserNum)
		ans += (ed.uniqueUserNum-minUniqueUserNum)/(maxUniqueUserNum-minUniqueUserNum)*0.2;
		if(maxCommentNum>minCommentNum)
		ans += (ed.commentNum - minCommentNum)/(maxCommentNum-minCommentNum)*0.2;
		if(maxRepostNum>minRepostNum)
		ans += (ed.repostNum-minRepostNum)/(maxRepostNum-minRepostNum)*0.2;
		if(maxVUserNum>minVuserNum)
		ans += (ed.vUserNum-minVuserNum)/(maxVUserNum-minVuserNum)*0.2;
		System.out.println(maxVUserNum + " : " + minVuserNum + " : " +ed.vUserNum);
		System.out.println(ans);
		return ans*100;
	}
	public double getSentimentD(EventDynamic ed){
		if(maxSentimentTendency>minSentimentTendency)
	...
	public double getActiveD(EventDynamic ed){
		if(maxActiveDegree>minActiveDegree)
